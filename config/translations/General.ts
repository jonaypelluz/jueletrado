import { Translations } from '@models/interfaces';
import { LoadingMessagesType } from '@models/types';

const LoadingMessages: LoadingMessagesType = {
    es: [
        'Preparando las letras para ti...',
        'Ordenando el abecedario, un momento...',
        'Las palabras están tomando asiento, ya casi...',
        'Cargando el diccionario, paciencia...',
        'Las tildes se están colocando en su sitio...',
        'Reuniendo todas las vocales y consonantes...',
        'Organizando las palabras por orden alfabético...',
        'Llenando el tablero de letras, casi listo...',
        'Los niveles se están preparando para el juego...',
        'Un segundo, estamos afinando la ortografía...',
    ],
    en: [
        'Getting the letters ready for you...',
        'Sorting the alphabet, one moment...',
        'Words are taking their seats, almost there...',
        'Loading the dictionary, hang tight...',
        'Accent marks are finding their place...',
        'Gathering all the vowels and consonants...',
        'Arranging words in alphabetical order...',
        'Filling the board with letters, almost done...',
        'Levels are getting ready for the game...',
        'One second, fine-tuning the spelling...',
    ],
};

const GeneralTranslations: Translations = {
    en: {
        errorTitle: 'Ops!',
        errorDescription: 'Words not found!',
        errorMessageTitle: 'Go to the beginning to look for the letters',
        // prettier-ignore
        errorMessageDescription: 'If the letters are not found and the error continues to occur, let\'s try to delete all the downloaded content.',
        errorMessageAction: 'Delete downloaded content',
        privacyTitle: 'Privacy Policy',
        cookiesTitle: 'Cookies Policy',
        cookiesSettings: 'Cookie settings',
        cookiesConsent:
            'This website uses cookies to analyze traffic. By continuing to browse, you accept their use.',
        cookiesAccept: 'Accept',
        cookiesDecline: 'Decline',
        cookiesDelete: 'Delete consent',
        cookiesConsentCurrent: 'Current consent',
        cookiesConsentAccepted: 'Accepted',
        cookiesConsentRejected: 'Rejected',
        cookiesMoreInfo: 'Learn more',
        gamesAllGames: 'All games',
        gameCheckWord: 'Check word',
        // prettier-ignore
        gameMissed: 'You didn\'t see',
        gamePlay: 'Start',
        gameSelectLevel: 'Choose a level to play',
        gameRulesAdditionalRules: 'Additional rules:',
        gameRulesGoaloftheGame: 'Goal of the Game:',
        gameRulesHowtoPlay: 'How to Play:',
        gameRulesRules: 'Rules:',
        gameRulesTips: 'Tips:',
        gameSeconds: 'seconds',
        gameSpeed: 'Speed',
        gameSpeedLevel: 'Level {speed}',
        donateTitle: 'Help keep Jueletrado alive',
        donateAction: 'Support the project',
        gameLoadError: 'Failed to load definitions. Please try again.',
        gameQuizWord: 'The definition of {quizWord} is:',
        gameQuizWordChoose: 'Choose another letter',
        gameQuizWordNextWord: 'Next word',
        gameWordBuilderGenerate: 'Generate other letters',
        gameWordBuilderInvalid: 'Not a valid word',
        gameWordBuilderDeleteLast: 'Delete',
        gameWordBuilderClearAll: 'Clear',
        gameCrossWordGenerating: 'Generating crossword...',
        gameCrossWordComplete: 'Congratulations! You completed the crossword!',
        gameAccentNoAccent: 'No accent mark',
        gameAccentResultPerfect: 'Congratulations! No mistakes!',
        gameAccentResultFew: 'You only made {count} mistake(s)!',
        gameAccentResultMany: 'You should study a bit more!',
        gameQuizFinishedScore: 'You got {score} out of {total} correct!',
        headerHome: 'Home',
        headerGames: 'Games',
        headerRules: 'Spelling Rules',
        headerRulesDescription:
            'Here are some essential rules for writing correctly, including punctuation, grammar, and the proper use of words.',
        headerWordOfTheDay: 'Word of the Day:',
        headerWordOfTheDayUrl: 'https://www.dictionary.com/browse/',
        homeLevel: 'Level:',
        homeChoseLevel: 'Choose the level',
        homeLoadLevels: 'Load levels',
        incorrectWords: 'Incorrect words:',
        levelBeginner: 'Beginner',
        levelIntermediate: 'Intermediate',
        levelAdvanced: 'Advanced',
        mainTitle: 'Jueletrado',
        mainDescription: 'Where playing and learning to write well go hand in hand',
    },
    es: {
        errorTitle: '¡Ops!',
        errorDescription: 'Lo sentimos, ¡no encontramos las palabras que buscas!',
        errorMessageTitle: 'Ir al inicio a buscar las letras',
        errorMessageDescription:
            'Si no se encuentran las letras y el error sigue ocurriendo probemos a borrar todo el contenido descargado.',
        errorMessageAction: 'Borrar contenido descargado',
        privacyTitle: 'Política de Privacidad',
        cookiesTitle: 'Política de Cookies',
        cookiesSettings: 'Configuración de cookies',
        cookiesConsent:
            'Este sitio web utiliza cookies para analizar el tráfico. Al continuar navegando, aceptas su uso.',
        cookiesAccept: 'Aceptar',
        cookiesDecline: 'Rechazar',
        cookiesDelete: 'Eliminar consentimiento',
        cookiesConsentCurrent: 'Consentimiento actual',
        cookiesConsentAccepted: 'Aceptado',
        cookiesConsentRejected: 'Rechazado',
        cookiesMoreInfo: 'Más información',
        gamesAllGames: 'Todos los juegos',
        gameCheckWord: 'Comprobar palabra',
        gameMissed: 'No viste',
        gamePlay: 'Jugar',
        gameSelectLevel: 'Elige un nivel para jugar',
        gameRulesAdditionalRules: 'Reglas Adicionales:',
        gameRulesGoaloftheGame: 'Objetivo del Juego:',
        gameRulesHowtoPlay: 'Cómo Jugar:',
        gameRulesRules: 'Reglas',
        gameRulesTips: 'Consejos:',
        gameSeconds: 'segundos',
        gameSpeed: 'Velocidad',
        gameSpeedLevel: 'Nivel {speed}',
        donateTitle: 'Ayuda a mantener Jueletrado vivo',
        donateAction: 'Apoya el proyecto',
        gameLoadError: 'Error al cargar las definiciones. Por favor, inténtalo de nuevo.',
        gameQuizWord: 'La definición de {quizWord} es:',
        gameQuizWordChoose: 'Elegir otra letra',
        gameQuizWordNextWord: 'Siguiente palabra',
        gameWordBuilderGenerate: 'Generar otras letras',
        gameWordBuilderInvalid: 'Palabra no válida',
        gameWordBuilderDeleteLast: 'Borrar',
        gameWordBuilderClearAll: 'Limpiar',
        gameCrossWordGenerating: 'Generando crucigrama...',
        gameCrossWordComplete: '¡Enhorabuena! ¡Has completado el crucigrama!',
        gameAccentNoAccent: 'No lleva tilde',
        gameAccentResultPerfect: '¡Enhorabuena! ¡Sin ningún error!',
        gameAccentResultFew: '¡Solo has fallado {count} vez/veces!',
        gameAccentResultMany: '¡Deberías estudiar un poco más!',
        gameQuizFinishedScore: '¡Acertaste {score} de {total}!',
        headerHome: 'Inicio',
        headerGames: 'Juegos',
        headerRules: 'Normas de ortografía',
        headerRulesDescription:
            'Reglas esenciales para escribir correctamente, incluyendo puntuación, gramática y uso de palabras.',
        headerWordOfTheDay: 'Palabra del día:',
        headerWordOfTheDayUrl: 'https://dle.rae.es/',
        homeLevel: 'Nivel:',
        homeChoseLevel: 'Elige el nivel',
        homeLoadLevels: 'Cargar niveles',
        incorrectWords: 'Palabras incorrectas:',
        levelBeginner: 'Principiante',
        levelIntermediate: 'Intermedio',
        levelAdvanced: 'Avanzado',
        mainTitle: 'Jueletrado',
        mainDescription: 'Donde jugar y aprender a escribir bien van de la mano',
    },
};

export { GeneralTranslations, LoadingMessages };
